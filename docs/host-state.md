...
| Syslog targets | UniFi → Settings → Cybersecure → Traffic Logging (`:1514`), and Settings → System Logging (`:1515`) | Both point at `10.6.1.81`, Akron's `alloy-syslog` LoadBalancer, **by IP**, because no internal name has ever been shown to resolve on the console — see the row below and #316. Every site targets Akron across the WAN today, which is #246. |
