...
- Independent single-node K3s sites, each managed with **Flux CD + Kustomize + Helm**, sharing this one repo (Flux's standard multi-cluster monorepo pattern — no cluster federation, no shared control plane):
  ...
  - **Lottage** (remote, 2GB RAM) — **not a cluster.** k3s does not fit; it runs PiHole, nginx and an Alloy collector under docker compose from `bare-metal/lottage/`, deployed by hand with `deploy.sh` and no Flux. Clients there behave like clients anywhere else — same blocklists, same names, same certificate shape — and its files are read out of `base/` rather than restated, so a change made for the clusters reaches it on the next deploy. See [the runbook](docs/runbooks/lottage-bare-metal.md). Re-add it as a real site, if the hardware is ever upgraded, by copying `sites/eastbank/` and `clusters/eastbank/`.

    **`bare-metal/` is deliberately not `sites/`.** `scripts/validate/lib-sites.sh` discovers sites from the `sites/*/` glob and then reads their layers out of `clusters/<site>/`; a directory there with no Flux entry point breaks discovery for the whole pipeline.
