...
**The two Python programs are unit-tested, and nothing else here is executable.** `base/landing/app.py` and `base/pihole-sync/sync.py` ship as opaque ConfigMap payloads, so every other step passes with a syntax error in either file — step 13 is the only thing that reads them. Tests live in `tests/`, use stdlib `unittest` (no pytest, so no new pin in the workflow), and load each program by path via `tests/loader.py` because neither is a package. Both files previously carried a hand-rolled self-check that nothing ever ran; if you add executable code, add its test to `tests/` rather than a `--self-check` flag.
...
- **Claude does not touch any site. This is a boundary, not a capability gap.** No `kubectl`, no `flux`, no SSH, no HTTP requests to site services — write the commands out for the user to run and ask for the output. The separation between local dev and production is deliberate, and it is also how the user learns the system.
  ...
  Do not assume the network prevents it. The dev machine is on a separate VLAN but *is* routed to Akron: it resolves through Akron's PiHole (`10.6.1.53`) and reaches Akron's Traefik, so a `curl` at `grafana.akron.internal.zerpzorp.com` will succeed. Restraint is the control here, not the network.

  **The one exception is read-only observability queries, through the `grafana` MCP server and nothing else.** It runs `mcp-grafana --disable-write` against a Viewer-scoped service account ([runbook](docs/runbooks/grafana-query-access.md)), so it reads Prometheus and Loki and can do nothing else. Ask before the first query in a session; after that, iterate freely — collapsing the propose/paste/interpret round trip is the entire point of #220, and asking per query gives it back.

  The exception is the *mechanism*, not the destination. A raw `curl` at Grafana is still out, even for a read: the MCP server is what makes "read-only" a property of the tool rather than of Claude's good intentions. Everything cluster-level, and anything that mutates state anywhere, stays out unconditionally.
...
**Collect deliberately, and treat the existing collection config as unreviewed.** Most of `alloy-metrics.alloy`'s scrape jobs and keep/drop rules were written early, while the limiting factors were still unknown, and were accepted without much scrutiny. They are not a considered baseline until the review in #381 settles them; update this paragraph when it does. The rules for what is collected:

- **Don't collect what a query can derive.** If a series can be reproduced, or closely approximated, with query or presentation logic over series already collected, it isn't worth collecting just in case. That includes a second source of the same signal. The exception is a derived value that is used routinely, such as a dashboard panel or a frequently checked query: there, collecting it (or a recording rule) can be cheaper than recomputing it on every refresh. The cost that matters is CPU and memory on the collectors and on Prometheus, not storage.
- **No fiddly variants.** Slight variations (extra breakdowns, labels, buckets) add cost without adding answers.
- **Intervals as short as the resources allow.** Shorter is better. A longer interval is a concession to the hardware, not a goal, so lengthen one only where it frees resources that are actually needed, and check that the `rate()` windows in dashboards and queries still get enough samples.

A metric gets added in one of two ways:

- **As a base metric.** Collecting ahead of need is fine. The difficulty is that the standard set for production engineering isn't known here yet, so the base set is expected to grow as investigations show what is actually needed. That is how it's meant to be found, not a failure of planning. A base metric should answer a question that will come up again, and its drop/keep rule should say which one.
- **Temporarily, during an active investigation.** While actively working a problem, collecting something to make the decision easier is acceptable even if it isn't a base metric yet. Open an issue in the same PR to re-evaluate it later: promote it to a base metric, or roll it back. Without that issue, a temporary metric silently becomes permanent.
...
DNS is free too: `base/dns/dnsmasq-base.conf` points `*.${SITE_DOMAIN}` at Traefik, so an `IngressRoute` is the whole of adding a service. The exceptions are hosts that are *not* behind Traefik, which need an explicit override or the wildcard swallows them. The node and the gateway are in `dnsmasq-base.conf` itself — both records are entirely variables, so every site gets them. A site-specific one, like Akron's syslog LoadBalancer, goes in `sites/<site>/infrastructure/site.conf`.
